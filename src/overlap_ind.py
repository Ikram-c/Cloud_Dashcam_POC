class IntervalUnionQuery:
    ...
    def modify_interval(self, i, k, offset, x_coord, rect_idx):
        for y in range(i, k):
            ...
            if old_count != new_count:
                self.sweep_events.append((x_coord, y, old_count, new_count, set(self.active_rectangles[y])))
    ...
    def find_overlaps(self):
                ...
                for count in range(old_count, new_count, -1):
                    if count >= 2 and y in active_regions.get(count, {}):
                        ...
                        if x - start_x > 0:
                            ...
                            y_high = (self.y_coords[y + 1] if y + 1 < len(self.y_coords) else self.y_coords[y])
        ...
        self.overlaps.sort(key=lambda x: (-x[4], x[0], x[2]))
...
def find_overlapping(rectangles):
    ...
    y_coordinates = sorted(list(set([y for rect in normalized_rectangles for y in rect[1::2]])))
    y_intervals = [y_coordinates[i + 1] - y_coordinates[i] for i in range(len(y_coordinates) - 1)]
    ...
    # Modified to return raw overlap indices for exact edge-to-zone matching
    overlapping_indices = []
    for (start_x, end_x, y_start, y_end, count, rect_indices) in sorted(interval_query.overlaps, key=lambda x: (-x[4], x[0])):
